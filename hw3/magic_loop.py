...
# Code based on Rayid's code for the course on Machine Learning
# Citation: https://github.com/rayidghani/magicloops
...
def clf_loop(models_to_run, clfs, grid, X, y):
    results_df =  pd.DataFrame(columns=('model_type','clf', 'parameters', 'auc-roc', 'p_at_10', 
        'p_at_20', 'r_at_10', 'r_at_20', 'f1_at_10', 'f1_at_20', 'train_time', 'pred_time'))
    confusion_matrix = {}
    for n in range(1, 2):
        ...
        for index,clf in enumerate([clfs[x] for x in models_to_run]):
            ...
            for p in ParameterGrid(parameter_values):
                try:
                    ...
                    predicting_time = time.time() - start_time_pred

                    cm = metrics.confusion_matrix(y_test, predicted)

                    confusion_matrix[models_to_run[index]] = cm
                    ...
                    # If running un Jupyter Notebook, then, you will see the plots, but activate the fuction at the start of the file.
                    if NOTEBOOK == 1:
                        plot_precision_recall_n(y_test,y_pred_probs,clf)
                        # print(metrics.confusion_matrix(y_test, y_pred_probs))
                        # print(metrics.classification_report(y_test, y_pred_probs))
                except:
                    print('Error:')
                    continue
    return results_df, confusion_matrix
...
def main(df, size): 
    ...
    results_df, matrixes = clf_loop(models_to_run, clfs,grid, X,y)
    if NOTEBOOK == 1:
        results_df
    results_df.to_csv('results.csv', index=False)

    return results_df, matrixes
