...
# Read
# 5/5
# Consider adding optional params for pd.read_csv

# Explore
# 7/10
# Use more bins for the histogram of age. The debt ratio plot has an x-axis of "Ages" ... Should be more careful with that. Also why is the plot only a single column?
# Is this because of outliers or not enough bins? In any case, this plot needs to 
# be tweaked because right now it's not informative at all. You can do more for the explor
# ation.

# Preprocess
# 3/6
# -3 for not handling fill_na for categorical variables differently. This function
# will error out for string column that have nan values. Also, categorical
# variables that are represented as integers wont have the right filled in value(b/c
# it will be a non-integer).

# Generate Features:
# 6/6
# Consider using pandas functions like: pd.cut, pd.get_dummies for discretizing/binarizing features
...
# Total: 34/40
# Reasonable job overrall.
# You should put the functions you wrote in a separate file from the one where
# you actually perform the data analysis/modeling.
